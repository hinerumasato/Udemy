...
public class FacebookAuthController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        UserModel userModel = new UserModel(email, null, firstName, lastName, null, picture, true, LoginType.FACEBOOK, Role.USER);
        if(!userService.exists(userModel)) {
            userService.insert(userModel);
        }

        userModel = userService.findByUsernameAndLoginType(userModel.getUsername(), LoginType.FACEBOOK);
